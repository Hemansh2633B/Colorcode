...
use crate::types::{ColorCodeError, ColorMatrix, DEFAULT_MODULE_SIZE, DEFAULT_QUIET_ZONE, EccLevel};
...
impl Encoder {
    ...
    pub fn encode_bytes(&self, data: &[u8]) -> Result<EncodedCode, ColorCodeError> {
        ...
        let capacity_bytes = layout::payload_data_capacity_bytes(self.options.ecc_level);
        if compressed.len() > capacity_bytes {
            return Err(ColorCodeError::PayloadTooLargeBytes {
